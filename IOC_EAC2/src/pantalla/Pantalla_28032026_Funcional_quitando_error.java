...
public class Pantalla_28032026_Funcional_quitando_error {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Pantalla_28032026_Funcional_quitando_error window = new Pantalla_28032026_Funcional_quitando_error();
	 ...
	 */
	public Pantalla_28032026_Funcional_quitando_error() {
		initialize();
	}
	...
	private void initialize() {
		...
		comboBoxTipusDe.setToolTipText("INDIVIDUAL, EMPRESA");
		...
		btnEsborrarTot.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		        ...
		        // 2. Resetear desplegables al primer elemento ("- Selecciona -")
		        ...
		        // 3. Restaurar estados de los paneles
	...
	// MÉTODO DE VALIDACIÓN
			...
			private void validarDades() {
				try {
					...
					// 2. NUEVO: Validar que se haya escogido un Pack (no el índice 0)
			        if (comboBoxPackDe.getSelectedIndex() == 0) {
			            mostrarError(comboBoxPackDe, "Has de seleccionar un pack de serveis.");
			            return;
			        }

			        // 3. NUEVO: Validar que se haya escogido un Tipus de Negoci (no el índice 0)
			...
			// METODO DE CALCULAR PREUS
			private void calcularPreus() {
				try {
			        textErrorAlertes.setText(""); // Limpiamos errores previos
			        String strDte = textDte.getText().trim();

			        // 1. Validar que no esté vacío
			        if (strDte.isEmpty()) {
			            mostrarError(textDte, "El camp Descompte no pot estar buit.");
			            return;
			        }

			        // 2. Validar que sea un número (puedes usar double si permites decimales, o int)
			        double descompte = Double.parseDouble(strDte);

			        // 3. Validar rango (por ejemplo, que no sea negativo ni mayor al 100%)
			        if (descompte < 0 || descompte > 500) { // Ajusta el máximo según tu ejercicio
			            mostrarError(textDte, "El descompte no és vàlid (ha de ser entre 0 i 500€).");
			            return;
			        }
			        
			     // 4. NUEVO: Validar que se haya escogido un Gestio de XXSS (no el índice 0)
			        if (comboBoxGestioDeXXSS.getSelectedIndex() == 0) {
			            mostrarError(comboBoxGestioDeXXSS, "Has de seleccionar una gestió de XXSS.");
			            return;
			        }
			        
			        // 5. NUEVO: Validar que se haya escogido un Publicitat SEM (no el índice 0)
			        if (comboBoxPublicitatSEM.getSelectedIndex() == 0) {
			            mostrarError(comboBoxPublicitatSEM, "Has de seleccionar una publicitat SEM.");
			            return;
			        }

			        // --- SI TODO ES CORRECTO, PROCEDER AL CÁLCULO ---
			        executarCalculs(descompte);

			    } catch (NumberFormatException ex) {
			        // Si el usuario escribe letras en lugar de números
			        mostrarError(textDte, "El descompte ha de ser un valor numèric.");
			    }
			}
			...
			private void executarCalculs(double descompte) {
				try {
			        ...
			        if (pack.equals("BASIC")) preuBase = 200;
			        else if (pack.equals("ESTANDAR")) preuBase = 450;
			        else if (pack.equals("FULL")) preuBase = 800;
			        else if (pack.equals("PREMIUM")) preuBase = 1200;
			        ...
			        // Nota: He afegit PIME i GRAN EMPRESA segons el teu llistat
			        double factorNegoci = 1.0;
			        String tipus = comboBoxTipusDe.getSelectedItem().toString();
			        
			        if (tipus.equals("AUTÒNOM")) factorNegoci = 0.8;
			        else if (tipus.equals("PIME")) factorNegoci = 1.0;
			        else if (tipus.equals("GRAN EMPRESA")) factorNegoci = 1.2;
			        // Si el teu combo només té "INDIVIDUAL/EMPRESA", adapta els textos aquí
			        ...
			        if (preuAbansImpostos < 0) preuAbansImpostos = 0; // Evitar preus negatius
